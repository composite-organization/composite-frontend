  ...
  return (
    <div className="relative w-[520px]">
      ...
      <div className="flex flex-col gap-5 px-4 pt-4 pb-5 bg-black-0 border-x border-b border-black-200 rounded-b-[20px]">
        ...
        {isEnded && <QuizResult correctRate={correctRate} />}
      </div>
      ...
      >
        <div className="flex flex-col gap-5 bg-black-50 rounded-2xl px-5 py-3">
          <div className="flex flex-col gap-3">
            {participantStatuses.map((status) => (
              <div
                key={status.choiceIndex}
                className="desc-regular text-black-500"
              >
                {status.choiceIndex + 1}: {status.participants.join(', ')}
              </div>
            ))}
          </div>
          <p className="desc-regular text-black-500">
